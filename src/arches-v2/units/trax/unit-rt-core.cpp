...
void UnitRTCore<NT, PT>::clock_rise()
{
	...
	//Each iteration is one issue slot. Slot i feeds pipline i, so the piplines are fed in lockstep
	//and stay balanced without needing an arbiter.
	...
	//Advance and drain each pipline exactly once per cycle. Clocking a LatencyFIFO is what advances
	//its latency counter, so this must stay out of the issue loop above or the modeled latency scales
	//down with the issue width.
	for(uint i = 0; i < NUM_ISECT_PIPLINES; ++i)
	{
