...
void UnitDRAMRamulator::clock_rise()
{
	...
	//Each controller is fed by NUM_REQ_PIPLINES sinks. Sink i drives its own pipline, which is clocked
	//exactly once per cycle so the configured latency is the latency actually modeled.
	for(uint i = 0; i < _request_network.num_sinks(); ++i)
	{
		...
		if(_request_network.is_read_valid(i) && req_pipline.is_write_valid())
			req_pipline.write(_request_network.read(i));
		req_pipline.clock();

		if(!req_pipline.empty()) busy = true;
		if(!req_pipline.is_read_valid()) continue;
		const MemoryRequest& request = req_pipline.peek();
...
void UnitDRAMRamulator::clock_fall()
{
	...
	for(uint controller_index = 0; controller_index < _controllers.size(); ++controller_index)
	{
		...
		//Drain up to NUM_REQ_PIPLINES returns per cycle so the return path is as wide as the request
		//path. The return network is already sized with a source per pipline; previously only the
		//first source of each controller was ever used, capping returns at one sector per cycle.
		for(uint k = 0; k < MemoryController::NUM_REQ_PIPLINES; ++k)
		{
			...
			if(_return_network.is_write_valid(source_index))
			{
				...
				_return_network.write(ret, source_index);
